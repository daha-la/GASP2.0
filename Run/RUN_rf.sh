...
# train
python ../Model_Architectures/randomforest/randomforest_train.py -i enzyme cid -c reaction -n 1000 -n $n_estimators -crit $criterion -maxD $max_depth -minS $min_samples_split -minL $min_samples_leaf -maxF $max_features -o ../Results/models/model_$identifier.rf.joblib.gz -a $chemFeat $encoding -rand $random_state < ../Data/trainset_GTP.tsv

# test
python ../Model_Architectures/randomforest/randomforest_test.py -m ../Results/models/model_$identifier.rf.joblib.gz -a $chemFeat $encoding < ../Data/testset_D1.tsv > ../Results/predictions/pred_$identifier.tsv
